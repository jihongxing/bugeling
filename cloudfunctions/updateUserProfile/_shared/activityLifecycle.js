...
function loadDetermineVerdict() {
  try {
    const mod = require('../autoArbitrate/index')
    if (mod && typeof mod.determineVerdict === 'function') return mod.determineVerdict
  } catch (err) {}
  ...
  throw new Error('activityLifecycle: determineVerdict loader failed')
}

const determineVerdict = loadDetermineVerdict()
