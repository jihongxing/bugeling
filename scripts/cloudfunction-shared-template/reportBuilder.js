...
function buildActivityReport(activity, participations, initiatorCredit) {
  ...
  return {
    ...
    budgetMin: Number(activity.budgetMin || 0),
    budgetMax: Number(activity.budgetMax || 0),
...
async function persistActivityReportSummary(db, collections, report) {
  const collection = db.collection(collections.ACTIVITY_REPORTS_SUMMARY)
  const existingRes = await collection.where({ activityId: report.activityId }).get()
  const existing = existingRes.data && existingRes.data[0]
  const baseData = Object.assign({}, report, {
    reusable: report.canReuse !== false,
  ...
  if (existing && existing._id) {
    ...
    return Object.assign({}, baseData, { _id: existing._id })
  }

  const addData = Object.assign({}, baseData, {
  ...
  return Object.assign({}, addData, { _id: addResult._id })
}

function buildCreateSeedFromReport(report) {
  const location = report.location || {}

  return {
    sourceReportId: report.reportId || report._id || report.activityId || '',
    activityId: report.activityId || '',
    templateType: report.templateType || 'other',
    title: report.title || '',
    summary: report.summary || '',
    budgetType: report.budgetType || 'aa',
    serviceFee: Number(report.serviceFee || 0),
    bondAmount: Number(report.bondAmount || report.depositTier || 0),
    minParticipants: Number(report.minParticipants || 0),
    maxParticipants: Number(report.maxParticipants || 0),
    identityHint: report.identityHint || '',
    meetingPointText: report.meetingPointText || location.name || '',
    realNameRequired: report.realNameRequired === true,
    genderLimit: report.genderLimit || 'none',
    allowAfterParty: report.allowAfterParty === true,
    safetyTags: Array.isArray(report.safetyTags) ? report.safetyTags : [],
    atmosphereTags: Array.isArray(report.atmosphereTags) ? report.atmosphereTags : []
  }
}

module.exports = {
  buildActivityReport,
  buildCreateSeedFromReport,
  persistActivityReportSummary,
